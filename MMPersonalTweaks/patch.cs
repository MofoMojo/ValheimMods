    ...
    class patch
    {
        ...
        */
        ...
        public class HarmonyPatch_DropItem
        {
            ...
            {
                bool enabled = Settings.DropBasedOnCameraDirection.Value;
                Plugin.Log($"DropBasedOnCameraDirection {enabled}");
            ...
            {
                // return if disabled
                if (!Settings.DropBasedOnCameraDirection.Value) return true;
                ...
                {
                    itemDrop = ItemDrop.DropItem(item, amount, (__instance as Character).transform.position + (__instance as Character).transform.forward + (__instance as Character).transform.up, (__instance as Character).transform.rotation);
                    itemDrop.GetComponent<Rigidbody>().velocity = ((__instance as Character).transform.forward + Vector3.up) * 5f;
                }
                //end our change..
